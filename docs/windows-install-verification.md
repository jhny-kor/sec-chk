...
- App Store 브랜드명에 맞춘 직접 배포용 설치파일 진입점은 `platforms/windows/scripts/build-koda-windows-installer.ps1`입니다.
...
3. `platforms/windows/packaging/KODA.iss`는 `%LOCALAPPDATA%\KODA` 설치 경로와 Start Menu/Desktop 바로가기를 정의합니다.

## 이번 로컬(macOS)에서 실제 확인한 항목

- ✅ 테스트 스위트: `python3 -m unittest discover -s tests` (64 tests passed)
- ✅ Python 진입점 구문 확인: `PYTHONPATH=platforms/shared/python python3 -m compileall platforms/macos/scripts/koda-app.py platforms/shared/python/security_scanner/app.py`
...
- Windows 설치파일의 최종 실기 검증은 Windows 10/11 + Python 3.10+ + Inno Setup 6 환경에서 `platforms\windows\scripts\build-koda-windows-installer.ps1` 실행으로 확인해야 합니다.
