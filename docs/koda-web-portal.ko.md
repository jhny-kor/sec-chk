...
[![KODA 프로젝트, 입력 파일, 검사 기준과 범위를 선택하는 새 점검 화면](assets/koda-web/koda-new-scan.png)](assets/koda-web/koda-new-scan.png)

입력은 최대 1GiB까지 스트리밍으로 등록하며, 사용자는 검사 기준과 범위만 선택합니다.
오프라인 취약점 DB를 사용할 수 없으면 실행 전에 `점검 불가`와 원인을 표시합니다.
...
[![KODA 승인 공유 계정의 접근 허용과 시스템 관리자 설정](assets/koda-web/koda-admin-access.png)](assets/koda-web/koda-admin-access.png)

비교 결과는 항목별 검색·상태 필터와 CSV·JSON 내보내기를 제공합니다. 회원가입과
계정 승인은 Tracker에서만 수행하고, KODA 관리자는 승인된 공유 계정의 KODA 접근과
프로젝트 역할만 설정합니다.
...
점검 중에는 준비·스캔·마무리 단계와 진행률을 표시하며 취소할 수 있습니다. 완료·실패·
취소 회차는 동일한 입력과 최신 관리자 정책으로 다시 실행할 수 있습니다. 라이브러리
단계에는 실제 사용한 Grype 버전, 오프라인 DB 메타데이터, 조회 구성요소 수와 경고를
표시하므로 `취약점 0건`과 `DB 점검 실패`를 구분할 수 있습니다. KODA 입력 파일은
브라우저 메모리에 Base64로 올리지 않고 최대 1GiB까지 스트리밍 업로드합니다.

계정 가입과 승인은 KODA-SBOM-Tracker가 단일 원본으로 관리합니다. KODA 관리자 화면은
승인 기능을 중복 제공하지 않고, 공유 계정의 KODA 접근 차단·허용과 프로젝트 역할만
관리합니다. 역할은 기능별 체크박스, 점검 규칙은 규칙별 활성 체크박스로 설정합니다.

기능도는 정보 밀도, 제한된 강조색, 정적 SVG 접근성 원칙을 참고해 KODA 문서용으로
새로 작성했습니다. 외부 스크립트나 이미지 의존성은 없습니다.
