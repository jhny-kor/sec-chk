...
기존 라이브러리·소스코드·품질 분류 탭도 함께 제공합니다.
...
화면은 코드·비밀정보·보안설정·예방통제 카테고리만 실행합니다. 입력은 최대 1GiB까지
스트리밍으로 등록하며, 사용자는 검사 기준과 범위만 선택합니다. 오프라인 취약점 DB를
사용할 수 없으면 라이브러리 화면에서 실행 전에 `점검 불가`와 원인을 표시합니다.
