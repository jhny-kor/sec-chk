...
- 외부 SAST 보완: I-14, C-01, C-02, C-03, C-04 (함수 간 데이터 흐름·수명 분석)
...
SW49 소스 분석은 `.java`, `.xml`, `.js`, `.jsp`, `.html` 확장자만 대소문자
구분 없이 점검합니다. 그 밖의 확장자와 확장자가 없는 파일은 제외하며 웹,
호스트, OSV/CVE·SBOM, JAR/바이너리 분석은 실행하지 않습니다. 지원 파일의
전체 프로젝트 매니페스트를 먼저 고정하고 로컬 규칙을 실행합니다.
...
수동·미지원 기준의 쌍은 범위 확인용 최소 placeholder이며 정확도 측정 자료가 아닙니다.
