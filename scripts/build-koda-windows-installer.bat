...
if not exist "%BUILD_PS1%" (
  ...
  echo Extract the full repository, then run scripts\build-koda-windows-installer.bat.
  pause
  exit /b 1
)
...
if not exist "%REPO_DIR%\security_scanner\" (
  ...
  echo Extract the full repository first, then run scripts\build-koda-windows-installer.bat.
  pause
  exit /b 1
)
