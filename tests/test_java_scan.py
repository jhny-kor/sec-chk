...
class JavaScanTests(unittest.TestCase):
    ...
    def test_language_omitted_generates_toggle_and_explicit_language_is_fixed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertNotIn("server-library-report-detail.html", main_report)
            self.assertIn("Archives", main_report)
            ...
            self.assertLess(html_report.index("대외 비인가"), html_report.index('id="language-ko"'))
    ...
    def test_library_detail_expands_nvd_evidence_and_links_to_summary(self) -> None:
        ...
        self.assertIn("Fixed · Final", detail_html)
        self.assertIn("Final 추적", detail_html)
