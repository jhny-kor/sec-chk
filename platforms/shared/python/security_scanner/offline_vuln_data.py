...
def load_offline_data(
    ...
    knvd, knvd_info, knvd_dates, knvd_fatal = _load_knvd(knvd_source, requested, warnings)
...
def _load_knvd(
    ...
    requested: set[str],
    warnings: list[str],
) -> tuple[dict[str, tuple[dict[str, object], ...]], DataSource, tuple[str, ...], bool]:
    if source is None:
    ...
    try:
        ...
        if isinstance(notices, list):
            for notice in notices:
                if not isinstance(notice, dict) or not isinstance(notice.get("cve_ids"), list):
                    continue
                for cve_id in notice["cve_ids"]:
        ...
        return {key: tuple(value) for key, value in records.items()}, _source_info(source, "loaded", {"name": "knvd", "generated_at": payload.get("generated_at", "")}), dates, False
    except (OSError, EOFError, json.JSONDecodeError, UnicodeDecodeError, AttributeError, TypeError, ValueError) as exc:
...
def _source_info(source: Path, status: str, metadata: dict[str, object] | None = None) -> DataSource:
    return DataSource(str(source), _hash_source(source), status, metadata or {})
