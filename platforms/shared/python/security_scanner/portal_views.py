...
.progress{{height:10px;background:#e8edf2;border-radius:999px;overflow:hidden}}.progress span{{display:block;height:100%;background:var(--teal);transition:width .25s}}.stage-grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px}}.stage-card{{padding:16px}}.stage-card strong{{display:block;font-size:18px;margin:8px 0}}
...
<div class='step'><h2>2. 입력 등록</h2><div class='drop-zone'><p><strong>소스 폴더·압축파일·JAR/WAR</strong></p><input id='file' type='file'><p id='file-note' class='muted'>최대 1 GiB</p><div class='progress' aria-label='업로드 진행률'><span id='upload-progress' style='width:0'></span></div><p id='upload-state' class='muted'></p><button id='upload' type='button'>선택 파일 등록</button></div><label>등록된 입력<select id='input'></select></label></div>
...
json('/koda/api/v1/standards').then(x=>{{standards=x;standardBox.innerHTML=x.map(v=>`<option value="${{v.id}}">${{v.labels.ko||v.labels.en||v.id}}</option>`).join('');renderCategories()}});
...
def run_page(run: dict, *, admin: bool) -> str:
    ...
    run_action = "<button id='retry' type='button'>다시 실행</button>" if terminal else "<button id='cancel' type='button'>취소</button>"
    body = f"""<div class='toolbar' style='justify-content:space-between;margin-bottom:16px'><div><span id='run-status' class='status status-{esc(run['status'])}'>{esc(run['status'])}</span> · {esc(run['standard'])} / {esc(run['standard_category'])}</div><div class='toolbar'>{run_action}<a class='button' href='/koda/compare?right={esc(run['run_id'])}'>이전 회차와 비교</a><a class='button' href='/koda/scans/new'>새 분석 회차</a>{exports}</div></div>
<section class='panel' style='padding:16px;margin-bottom:16px'><div class='toolbar' style='justify-content:space-between'><strong id='run-stage'>{esc(run.get('stage', run.get('status', 'queued')))}</strong><span id='run-progress-text'>{esc(run.get('progress', 0))}%</span></div><div class='progress'><span id='run-progress' style='width:{esc(run.get('progress', 0))}%'></span></div></section>
